...
[adr.md, D-A9](pipeline-adr.md). O corpo do PR **declara** a renomeação.
...
(D4 do PRD; correção do 451 para 415 medida no ADR D-A10), sobrecarga
`TFunc<T,R>` do factory, procedure global, correção do bug `{$IFDEF FCP}`
em `ModernSyntax.inc:256`.
...
- `Source/ModernSyntax.Std.pas`, `Source/ModernSyntax.DotEnv.pas` (F-02
  do intake; fora do escopo desta linha).
- Fixtures DUnitX existentes em `Test Delphi/EclbrSystem/` — a nova
  convenção de duas cascas vale para o que **este** ciclo entrega.

## Notas de implementação

- **`uses` da unit nova:** somente `SysUtils`. Trazer outra unit da
  biblioteca reintroduz `reference to` ou o `.inc` transitivamente
  (medido no relatório de investigação).
- **Ramificação:** `{$IFDEF FPC} ... {$ELSE} ... {$ENDIF}` **direto**
  no arquivo, **jamais** via `{$I ModernSyntax.inc}` (R3 do PRD).
- **Wrappers de método de objeto:** guardar o `TMethod` (ou o próprio
  ponteiro de método tipado) em campo privado da classe wrapper.
...
- **DUnitX vs FPCUnit:** DUnitX não está vendorizado (medido); FPCUnit
  é nativo do FPC 3.2.2 (medido). Não tentar unificar frameworks — a
  unificação está na unit de cenários, não no framework.
...
- [ ] `lazbuild --build-mode=Debug-i386 "Test FPC/EclbrSystem/PTestModernCallback.lpi"` (executado pelo orquestrador na máquina do autor) compila.
- [ ] `lazbuild --build-mode=Debug-x86_64 "Test FPC/EclbrSystem/PTestModernCallback.lpi"` idem.
