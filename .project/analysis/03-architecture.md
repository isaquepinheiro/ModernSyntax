...
| `ModernSyntax.Currying` | `TCurrying` partial-application record; `TPipeline<T>` combinator; `TMemoizedCache<T,U>`; 12 `INumeric<T>` implementors for numeric types | none |
...
`Source/ModernSyntax.Match.pas:63` (generic record) and line 211 (companion class). The record holds a state machine via `TMatchSession` enum (`sMatch, sGuard, sCase, sDefault, sTryExcept`, line 57) and a `TCaseGroup` dictionary (alias `TDictionary<TValue,TValue>`, line 60) keyed by `TCaseType` (14-variant enum, lines 32-51). The companion `TMatch` class (line 211) uses a `class var FMatch: TValue` to shuttle the result out of the record's value-copy chain. This two-type design (record for chaining, class for shared state) is the core structural decision in the matching subsystem.
...
`Source/ModernSyntax.Currying.pas:163, 97, 134`. `TCurrying` is a record providing partial application over anonymous method references. `TPipeline<T>` (line 97) is a generic record enabling `.Pipe(f).Pipe(g)…` composition chains. `TMemoizedCache<T,U>` (line 134) is a ref-counted class implementing `ICleanup` (line 33) that stores computed results in a `TDictionary`. 12 concrete `INumeric<T>` implementors (lines 390–766) cover all numeric types from `Byte` to `Currency`.
...
Consumer code calls `TMatch<T>.Value(x)` → chains `.CaseEq(v, proc)` / `.CaseIf(cond: Boolean, proc)` / `.Default(proc)` → calls `.Execute`. Each chaining method updates `FCases` in the record and advances `FSession`; `.Execute` dispatches to one of the 14 `_Matching*` private methods (lines 78–95), which walk `FCases` and invoke the stored anonymous method. The final value is stored in `TMatch.FMatch` (class var, line 213) and extracted via `TMatch.Value<T>`.
